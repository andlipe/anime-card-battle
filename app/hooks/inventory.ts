...
export const useInventory = () => {
    ...
    function addCardToInventory(card: Card) {
        const indexOfCardAlreadyOnInventory = inventory.findIndex(inventoryCards => card.id === inventoryCards.id)
        if(indexOfCardAlreadyOnInventory !== -1) {
            setInventory(inventory.map(inventoryCard => inventoryCard.id === card.id ? {
                ...inventoryCard,
